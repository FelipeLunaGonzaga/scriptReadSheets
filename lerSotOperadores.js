...
function processarArquivoCSV(caminhoArquivo) {
  ...
  const diaDaSemana = data ? getDiaDaSemanaEmPortugues(data) : '';

  // Exibe os horários e respectivos controladores
  console.log(`Dia: ${data} (${diaDaSemana})`);
  horariosPermitidos.forEach((horario, index) => {
    console.log(`${horario}: ${linhaDeSetores[index]}`);
  });
...
// Lê todos os diretórios em "./JANEIRO" e processa cada um
fs.readdirSync("./JANEIRO").forEach(diretorio => adicionarPlanilhasDeDiretorio(`./JANEIRO/${diretorio}`));
