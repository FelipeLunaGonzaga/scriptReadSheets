...
function adicionarPlanilhasDeDiretorio(diretorio) {
  ...
  arquivosFiltrados.forEach((arquivo) => {
    ...
    if (isPrimeiraLinha) {
      fs.appendFileSync('novembroSOTSetores.csv', csv + "\n", "utf8");
      isPrimeiraLinha = false;
    } else {
      const csvSemHeader = csv.split("\n").slice(1).join("\n");
      fs.appendFileSync('novembroSOTSetores.csv', csvSemHeader + "\n", "utf8");
    }
  });
...
fs.readdirSync("./PASTAPARAEXTRAIR").forEach(diretorio => adicionarPlanilhasDeDiretorio(`./PASTAPARAEXTRAIR/${diretorio}`));
